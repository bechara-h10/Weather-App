...
let tempType = 'Celsius'
  ...
  if(new Date().getHours() > 6 && new Date().getHours < 20){
    document.body.style.backgroundImage = 'url(images/night-mountains-minimalist-8k-wo-1920x1080.jpg)'
  } else{
    document.body.style.backgroundImage = `url(images/forest_mountains_sunset_cool_weather_minimalism.jpg)`
  }
...
function updateTodayInfo(input,tempType) {
  ...
  getWeatherInfo(input).then((weatherInfo) => {
    ...
    tempMin.innerText = `${kelvintoFahrenheit(weatherInfo.main.temp_min)} °F`;
    tempMax.innerText = `${kelvintoFahrenheit(weatherInfo.main.temp_max)} °F`;
